...
export function registerCrmRoutes(app: any) {
  ...
  // Este endpoint permite criar uma sessão de teste sem autenticação
  // para validar o fluxo de atualização de leads
  if (process.env.NODE_ENV !== "production") {
    r.post("/test/create-session", async (req, res) => {
      try {
        ...
        res.json({ ok: true, token, user: { id: userId, name: "Usuário de Teste", email: "test@sams.local", role: "vendedor" } });
      } catch (e: any) {
