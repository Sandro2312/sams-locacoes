...
(function () {
  ...
  function renderFilteredDocuments(modal) {
    ...
    const filtered = documents.filter((doc) => {
      const haystack = [doc.nome, doc.nome_arquivo_original, doc.classificacao, doc.categoria_dossie, doc.observacao, doc.tags, doc.tags_dossie].join(' ').toLocaleLowerCase('pt-BR');
    ...
    return `<li class="rounded-lg border border-gray-200 bg-white p-3">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div class="min-w-0"><div class="font-medium text-gray-900 break-words"><i class="fas fa-file-alt mr-2 text-indigo-600"></i>${escapeHtml(doc.nome)}</div>
          <div class="mt-1 flex flex-wrap gap-2 text-xs text-gray-500"><span>${fileName}</span>${doc.classificacao ? `<span class="rounded-full bg-indigo-50 px-2 py-0.5 font-medium text-indigo-700">${escapeHtml(String(doc.classificacao).split('_').join(' '))}</span>` : ''}${doc.categoria_dossie ? `<span class="rounded-full bg-violet-50 px-2 py-0.5 font-medium text-violet-700">${escapeHtml(String(doc.categoria_dossie).replace(/_/g, ' '))}</span>` : ''}${doc.tamanho_bytes ? `<span>${formatBytes(doc.tamanho_bytes)}</span>` : ''}${doc.anexado_em ? `<span>Anexado em ${new Date(doc.anexado_em).toLocaleDateString('pt-BR')}</span>` : ''}</div>
          ${doc.tags_dossie ? `<div class="mt-2 flex flex-wrap gap-1">${String(doc.tags_dossie).split(',').filter(Boolean).map((tag) => `<span class="rounded-full bg-violet-50 px-2 py-0.5 text-xs font-medium text-violet-700">#${escapeHtml(tag)}</span>`).join('')}</div>` : ''}
