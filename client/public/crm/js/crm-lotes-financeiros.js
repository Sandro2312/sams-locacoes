...
(function () {
  ...
  function renderLoteWorkspace() {
    ...
    const confirmationDock = !confirmed && itens.length ? `<aside data-finance-batch-confirmation-dock class="mt-5 rounded-xl border-2 border-emerald-400 bg-emerald-50 p-4 sm:p-5" aria-label="Confirmar lançamentos do lote"><div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p class="text-xs font-bold uppercase tracking-wide text-emerald-700">Etapa final</p><p class="mt-1 text-sm font-bold text-emerald-950">${itens.length} item(ns) pronto(s) para virar lançamentos reais</p><p class="mt-1 text-xs leading-5 text-emerald-800">A confirmação cria os registros pendentes no Financeiro e não pode ser desfeita automaticamente.</p></div><button type="button" data-finance-batch-action="confirmar" class="w-full shrink-0 rounded-lg bg-emerald-700 px-5 py-3 text-sm font-bold text-white hover:bg-emerald-800 focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:ring-offset-2 sm:w-auto"><i class="fas fa-check-double mr-2"></i>Confirmar e criar lançamentos</button></div></aside>` : '';
