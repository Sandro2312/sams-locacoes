...
(function () {
  ...
  const state = { items: [], loading: false, filters: { eventoId: '', clienteId: '' } };
  ...
  function render() {
    ...
    return `
      <section class="space-y-5" aria-label="Resultado por stand">
        <div class="bg-white rounded-lg shadow p-4 md:p-6">
          <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p class="mt-1 text-sm text-gray-600">Apure custos diretos e receitas por cliente dentro de cada feira.</p>
            </div>
          ...
          <div class="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div class="flex items-end"><button id="projeto-stand-refresh" type="button" class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50"><i class="fas fa-sync-alt mr-2"></i>Atualizar</button></div>
          </div>
        </div>
            ...
            <tbody id="projeto-stand-list" class="divide-y divide-gray-100">
              ${state.loading ? `<tr><td colspan="${manage ? 6 : 5}" class="px-4 py-8 text-center text-sm text-gray-500"><i class="fas fa-spinner fa-spin mr-2"></i>Carregando Projetos de Stand...</td></tr>` : state.items.length ? state.items.map((item) => `
                <tr class="hover:bg-gray-50">
                  ...
                  <td class="px-4 py-3"><div class="font-medium text-gray-900">${escapeHtml(item.cliente_nome || '-')}</div><div class="text-sm text-gray-500">${escapeHtml(item.nome || '-')} ${item.referencia_stand ? `· ${escapeHtml(item.referencia_stand)}` : ''}</div><div class="text-xs text-gray-400">${escapeHtml(item.codigo || '')}</div></td>
                  ...
                  <td class="px-4 py-3"><span class="inline-flex rounded-full px-2 py-1 text-xs font-semibold ${statusClass(item.status)}">${escapeHtml(statusLabel(item.status))}</span></td>
        ...
        <p class="text-xs text-gray-500">A margem exibida considera receitas vinculadas menos custos diretos vinculados. Custos compartilhados e rateios serão incluídos em uma etapa posterior e auditável.</p>
      </section>`;
  }
    ...
    return `<div id="projeto-stand-modal" class="fixed inset-0 z-[1000] flex items-start justify-center overflow-y-auto bg-black/50 p-4" role="dialog" aria-modal="true" aria-labelledby="projeto-stand-modal-title">
      <div class="my-4 w-full max-w-3xl rounded-xl bg-white shadow-2xl">
        <form id="projeto-stand-form" class="p-5 md:p-6">
          <div class="flex items-start justify-between gap-4"><div><h3 id="projeto-stand-modal-title" class="text-lg font-semibold text-gray-900">${item.id ? 'Editar' : 'Novo'} Projeto de Stand</h3><p class="mt-1 text-sm text-gray-600">Esta é a unidade que vincula evento, cliente, receitas e custos diretos.</p></div><button type="button" id="projeto-stand-cancel" class="text-xl text-gray-400 hover:text-gray-700" aria-label="Fechar">&times;</button></div>
          <div class="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <label class="block text-sm font-medium text-gray-700">Cliente *<select name="clienteId" required class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2">${clientOptions(item.cliente_id ?? item.clienteId, false)}</select></label>
